...
class IELManager(GenericManagerAgent):
    ...
    def __init__(self, config_file, endow, total_rounds=10, name="IELManager2",
                 base_cash = 0):
        ...
        for email, holdings in user_holdings.items():
            for market, h in holdings.assets.items():
                if email not in self.endow:
                    ...
                    holdings.assets[market].units_initial = int(self.endow[email][market.item][self.current_round])
            ...
            for email, holdings in user_holdings.items():
                for market in markets:
                    if email not in self.endow:
                        ...
                        holdings.assets[market].units_initial = int(self.endow[email][market.item][0])
...
if __name__ == "__main__":
    # Map the email of bot i to a dictionary, where each item is mapped to
    # an array w^i = [w_1^i, ..., w_T^i]. T is the total number of rounds
    endow = {'mtjandra@caltech.edu': {'A': [0, 10],
                                      'B': [10, 0]},
             'test1@test': {'A': [0, 10],
                            'B': [10, 0]},
             'test2@test': {'A': [0, 10],
